...
(function (global) {
  ...
  /* ── progress ring ─────────────────────────────────────────────── */
  /* Rendered on the dark premium hero gradient, so track/text use
     light, semi-transparent tones rather than the ink/muted palette. */
  function ring(pct, opts) {
    ...
    if (!color) color = p > 100 ? '#FF9C93' : p >= 80 ? '#DDB94F' : '#58C97D';
    ...
    return (
      ...
      '<text x="50%" y="' + (sub ? '46%' : '54%') + '" text-anchor="middle" font-size="' + (size * 0.21) + '" font-weight="800" fill="#FFFFFF">' + esc(label) + '</text>' +
      (sub ? '<text x="50%" y="63%" text-anchor="middle" font-size="' + (size * 0.115) + '" font-weight="650" fill="rgba(255,255,255,0.72)">' + esc(sub) + '</text>' : '') +
      '</svg>'
    );
  }
  ...
  function weeklyBars(values, opts) {
    ...
    values.forEach(function (v, i) {
      ...
      parts.push(
        ...
        (v > 0 ? (isMax ? '#C9A227' : '#8FD6A6') : '#E9E5D9') + '"/>'
      );
      parts.push(
        '<text x="' + (x + bw / 2).toFixed(1) + '" y="' + (y - 5).toFixed(1) + '" text-anchor="middle" font-size="9.5" font-weight="700" fill="' +
        (v > 0 ? '#12251A' : '#97A28E') + '">' + (v > 0 ? esc(opts.money ? opts.money(v) : fmtShort(v)) : '–') + '</text>'
      );
      parts.push(
        '<text x="' + (x + bw / 2).toFixed(1) + '" y="' + (H - 7) + '" text-anchor="middle" font-size="9.5" font-weight="650" fill="#63705F">' + esc(labels[i]) + '</text>'
      );
    });
  ...
  function trendBars(points, opts) {
    ...
    points.forEach(function (p, i) {
      ...
      parts.push(
        '<rect x="' + x.toFixed(1) + '" y="' + y.toFixed(1) + '" width="' + bw.toFixed(1) + '" height="' + h.toFixed(1) + '" rx="6" fill="' +
        (p.value > 0 ? (isCur ? '#0B3D22' : '#8FD6A6') : '#E9E5D9') + '"/>'
      );
      if (p.value > 0) {
        parts.push(
          '<text x="' + (x + bw / 2).toFixed(1) + '" y="' + (y - 5).toFixed(1) + '" text-anchor="middle" font-size="9" font-weight="700" fill="#12251A">' +
      ...
      parts.push(
        ...
        (isCur ? '#0B3D22' : '#63705F') + '">' + esc(p.label) + '</text>'
      );
    });
